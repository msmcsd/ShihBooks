...
namespace ShihBooks.ViewModels
{
    public partial class ManageEntitiesWithUrlViewModel : BaseViewModel
    {
        ...
        {
            var m = (CoreEntityWithUrl)await Shell.Current.CurrentPage.ShowPopupAsync(new ManageEntityWithUrlPopupPage());
            if (m is null)
            {
                return;
            }
        ...
        public override async Task UpdateEntityAsync()
        {
            ...
            var newSource = (CoreEntityWithUrl)await Shell.Current.CurrentPage.ShowPopupAsync(new ManageEntityWithUrlPopupPage(SelectedEntity.Name, SelectedEntity.ImageUrl));
            ...
            if (newSource.Name != origMerchantName ||
               ...
               (!string.IsNullOrWhiteSpace(newSource.ImageUrl) && string.IsNullOrWhiteSpace(origImageUrl)) ||
               (newSource.ImageUrl.ToLower() != origImageUrl.ToLower()))
            {
